...
export class UserFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //   name: new FormControl('', [Validators.required, Validators.minLength(5),validateNameWithSalutation]),
    ...
    this.user = this.fb.group({
      name: ['', [Validators.required, Validators.minLength(5), validateNameWithSalutation]],
...
//Creating a custom validator function for
//ReactiveForms
export function validateNameWithSalutation(
                            control: AbstractControl) {

  if (control.value.startsWith('Mr.') || control.value.startsWith('mr.')) {
    //return { salutationError: false };
    return null;
  } else {
    return { salutationError: true };
  }
}
